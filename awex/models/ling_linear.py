...
def _normalize_scale_inv_name(name: str) -> Tuple[str, bool]:
    suffix = "_scale_inv"
...
def _append_scale_inv(name: str, has_scale_inv: bool) -> str:
    if has_scale_inv:
        return f"{name}_scale_inv"
    return name


class BailingLinearMoeShardingStrategy(BailingMoeShardingStrategy):
...
def _build_mcore_converter_bailing_moe_linear():
    ...
    from awex.models.ling import _build_mcore_converter_bailing_moe

    BaseBailingMoeConverter = _build_mcore_converter_bailing_moe()

    class McoreToHFWeightConverterBailingMoeLinear(BaseBailingMoeConverter):
        def __init__(
            self,
            ...
            self.layer_group_size = int(
                _cfg_value(tf_config, "layer_group_size", 1) or 1
            )
        ...
        def _convert_lightning_attention_param(
            ...
            if "self_attention.pre_gate_norm.weight" in name:
                return [
                    ("attention.g_norm.weight", self._convert_g_norm_weight(parameter))
                ]
        ...
        def convert_param(
            ...
            if "self_attention" in remaining_name:
                ...
                return self._apply_fp8_quantization(converted)

            if (
                "input_layernorm.weight" in remaining_name
                or "g_norm.weight" in remaining_name
            ):
                return self._apply_fp8_quantization(
            ...
            if "mlp" in remaining_name:
                if "mlp.gate.weight" in name or "mlp.router.weight" in name:
                    ...
                    converted = [
                        (f"model.layers.{layer_number}.{gate_name}", gate_param)
                    ]
                    ...
                    converted = [
                        (f"model.layers.{layer_number}.{bias_name}", bias_param)
                    ]
                else:
                    converted = [
                        (f"model.layers.{layer_number}.{param_name}", param)
...
class SGlangToHFWeightConverterBailingMoeLinear(SGlangToHFWeightConverter):
    ...
    def _convert_layer_norm_param(
        ...
        base_name, has_scale_inv = _normalize_scale_inv_name(name)
        direct_params = {
        ...
        if base_name in direct_params:
            return [(_append_scale_inv(base_name, has_scale_inv), parameter)]
    ...
    def _convert_attention_param(
        ...
        if base_name in name_mapping:
            ...
            return [(_append_scale_inv(target_name, has_scale_inv), parameter)]

        if base_name in {
            "attention.query_key_value.weight",
            "self_attn.qkv_proj.weight",
        }:
            if self._fuse_qkv(base_name):
                return [
                    (
                        _append_scale_inv(
                            "attention.query_key_value.weight", has_scale_inv
                        ),
                        parameter,
                    )
                ]
        ...
        if base_name in {
            ...
            if self._fuse_qkv(base_name):
                return [
                    (
                        _append_scale_inv(
                            "attention.query_key_value.bias", has_scale_inv
                        ),
                        parameter,
                    )
                ]
